    ...
    <sm-container :full-width="true">
      ...
      <template v-if="!filteredReservations.length">
        ...
        >
          ...
          <template #header>
            <h2 class="sm-h3">{{ t('not-implemented') }}</h2>
          </template>
        ...
        <sm-table-thead>
          <sm-table-tr>
            <sm-table-th
              ...
              :width="tableWidth + 'px'"
              @sorting-table-order="sortByBookingReference"
            >
      ...
      <sm-pagination
        v-model:current-page="currentReservationsPage"
      ...
      <sm-dialog
        v-model:visible="searchFeedbackDialog"
        type="alert"
        :show-cancel="false"
        :show-confirm="false"
