...
import { SmSelect, SmSelectOption } from '@siteminder/sui-core/components/forms/sm-select'
...
import DirectBookingMedia from '@/components/direct-booking/direct-booking-media.vue'
...
const siteTypeOptions = ref<SmSelectOption[]>([
...
const statusOptions = ref<SmSelectOption[]>([
...
const singleTentImages: Ref<Image[]> = ref([])
const selectedSingleTentImages: Ref<string[]> = ref([])
const doubleTentImages: Ref<Image[]> = ref([])
const kingTentImages: Ref<Image[]> = ref([])
const queenTentImages: Ref<Image[]> = ref([])

const asyncFunction = (delay = 1000): Promise<void> => {
...
const uploadedFiles = (files: File[], siteType: string): void => {
  ...
  if (siteType === siteTypes[2]) {
    files.forEach((_, i) => {
      kingTentImages.value.push({
        src: `https://picsum.photos/1150/940?random=${kingTentImages.value.length + 1}`,
        id: `${i}${i}${i}`,
      })
    })
  }

  if (siteType === siteTypes[3]) {
    files.forEach((_, i) => {
      queenTentImages.value.push({
        src: `https://picsum.photos/1150/940?random=${queenTentImages.value.length + 1}`,
        id: `${i}${i}${i}`,
      })
    })
  }
...
const linkMaker = (): void => {
  for (let i = 0; i < 6; i += 1) {
    ...
    kingTentImages.value.push({
      src: `https://picsum.photos/id/${i + 10}/940`,
      id: `${i}${i}${i}`,
      isMainPhoto: i === 1,
      isExternal: i === 2,
      hasWarning: i === 3,
...
const removeKingTentImageHandler = (imageToRemove: Image): void => {
  kingTentImages.value = kingTentImages.value.filter(image => image.src !== imageToRemove.src)
}

const removeQueenTentImageHandler = (imageToRemove: Image): void => {
  queenTentImages.value = queenTentImages.value.filter(image => image.src !== imageToRemove.src)
}

const toggleKingTentMainPhoto = (imageToUpdate: Image): void => {
  kingTentImages.value = kingTentImages.value.map(image => ({
    ...image,
    isMainPhoto: image.src === imageToUpdate.src,
  }))
}

const toggleQueenTentMainPhoto = async (imageToUpdate: Image): Promise<void> => {
  queenTentImages.value = queenTentImages.value.map(image => ({
    ...image,
    isMainPhotoLoading: image.src === imageToUpdate.src,
  }))

  // Random delay between 0 and 1000ms
  const randomDelay = Math.floor(Math.random() * 1000)
  console.info('Simulating async operation with delay:', randomDelay)

  try {
    await asyncFunction(randomDelay)

    // Simulate a successful operation by updating the main photo
    queenTentImages.value = queenTentImages.value.map(image => ({
      ...image,
      isMainPhoto: image.src === imageToUpdate.src,
      isMainPhotoLoading: false,
    }))

    // Silent when successful, but up to products
    // toastService({
    //   type: 'success',
    //   message: t('main-photo-updated-toast-text'),
    //   showClose: true,
    // })
  } catch (error) {
    queenTentImages.value = queenTentImages.value.map(image => ({
    ...
    toastService({
      type: 'warning',
      message: t('async-operation-failed-toast-text'),
      showClose: true,
    })
  }
}
...
<template>
  <sm-section>
    <sm-container :full-width="true">
      ...
      <sm-select
        v-model:selection="statusFilter"
      ...
      <sm-section :id="siteTypeIds[0]">
        <h3>{{ siteTypes[0] }}</h3>
        <sm-media
          v-model:images="singleTentImages"
          :accept-mime="acceptMime"
          group="group1"
          data-sm-test-id="media-single-tent"
          @files-added="uploadedFiles($event, siteTypes[0])"
        >
          <template
            #media="{image}"
          >
      ...
      <sm-section :id="siteTypeIds[1]">
        <h3>{{ siteTypes[1] }}</h3>
        <sm-media
          v-model:images="doubleTentImages"
          :accept-mime="acceptMime"
          group="group1"
          data-sm-test-id="media-double-tent"
          :empty-state-drop="true"
          @files-added="uploadedFiles($event, siteTypes[1])"
        >
          <template
            #media="{image}"
          >
      ...
      <sm-section :id="siteTypeIds[2]">
        <h3>{{ siteTypes[2] }}</h3>
        <p>{{ t('media-hint-text-1') }}</p>
        <direct-booking-media
          v-model="kingTentImages"
          @remove-image="removeKingTentImageHandler"
          @set-main-photo="toggleKingTentMainPhoto"
          @files-added="uploadedFiles($event, siteTypes[2])"
        />
      </sm-section>

      <sm-section :id="siteTypeIds[3]">
        <h3>{{ siteTypes[3] }}</h3>
        <p>{{ t('media-hint-text-2') }}</p>
        <direct-booking-media
          v-model="queenTentImages"
          @remove-image="removeQueenTentImageHandler"
          @set-main-photo="toggleQueenTentMainPhoto"
          @files-added="uploadedFiles($event, siteTypes[3])"
        />
      </sm-section>

      <sm-dialog
        v-model:visible="editRateDialogVisible"
      ...
      <sm-dialog
        v-model:visible="copyToolDialogVisible"
