...
import { selectedLanguage, selectedWhiteLabel, Language, WhiteLabelColour, useGlobalSettings, themeMode, ThemeMode, SuiThemeProduct } from '@/composables/use-global-settings'
...
// - white-labelling: body .sm-horizontal-nav .sm-horizontal-nav-item--active .sm-horizontal-nav-item__content
// consider completely dropping support for white-labelling.css
// give a warning not to use white-labelling and sui-themes together!
// better yet, just remind people to stop using white-labelling.css completely.
const { language, whiteLabel, suiTheme, isWhiteLabelModeEnabled } = useGlobalSettings()
...
watch(whiteLabel, async (newValue) => {
  if (themeMode.value !== ThemeMode.WHITE_LABEL) {
    await import('@siteminder/sui-core/white-labelling/white-labelling.css')
